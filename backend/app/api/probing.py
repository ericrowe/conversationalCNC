...
from ..schemas.probing_schema import (
    ZProbeRequestSchema,
    CornerXYZProbeRequestSchema,
    BoreCenterProbeRequestSchema,
    BossCenterProbeRequestSchema,
)
...
@probing_bp.route("/z", methods=["POST"])
...
@probing_bp.route("/xyz", methods=["POST"])
def api_corner_xyz():
    data = request.get_json() or {}
    try:
    ...
    result = generate_corner_xyz_probe_macro(
        ...
        corner=payload.corner or "front_left",
        search_dist=payload.search_dist,
        fast_feed=payload.fast_feed,
        slow_feed=payload.slow_feed,
...
@probing_bp.route("/bore-center", methods=["POST"])
def api_bore_center():
    data = request.get_json() or {}
    try:
        payload = BoreCenterProbeRequestSchema(**data)
    except ValidationError as e:
        error_details = [{"loc": err.get("loc"), "msg": err.get("msg"), "type": err.get("type")} for err in e.errors()]
        return jsonify({"error": "Validation error", "details": error_details}), 400

    result = generate_bore_center_probe_macro(
        approx_diameter=payload.approx_diameter,
        tool_diameter=payload.tool_diameter,
        search_dist=payload.search_dist,
        fast_feed=payload.fast_feed,
        slow_feed=payload.slow_feed,
...
@probing_bp.route("/boss-center", methods=["POST"])
def api_boss_center():
    data = request.get_json() or {}
    try:
        payload = BossCenterProbeRequestSchema(**data)
    except ValidationError as e:
        error_details = [{"loc": err.get("loc"), "msg": err.get("msg"), "type": err.get("type")} for err in e.errors()]
        return jsonify({"error": "Validation error", "details": error_details}), 400

    result = generate_boss_center_probe_macro(
        approx_diameter=payload.approx_diameter,
        tool_diameter=payload.tool_diameter,
        search_dist=payload.search_dist,
        fast_feed=payload.fast_feed,
